...
int compare_results(const unsigned char *a, const unsigned int *b) {
    for (int i = 0; i < 4; ++i) {
...
__kernel void md4_crack(__global const unsigned int *target,
  ...
  unsigned char candidate[64]={0}; 
  candidate[56]=PWD_LEN*8;  
  candidate[PWD_LEN]=0x80;
  candidate[0]=buffer[0];
  candidate[1]=buffer[1];
  candidate[2]=buffer[2];
  ...
  do {
    ...
    if (compare_results(res, target)) {

      candidate[PWD_LEN]=0;
      printf("found: %s, password is: %s \n", candidate,solution);
